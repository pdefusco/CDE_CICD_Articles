...
trxBatchDf = spark.read.schema("credit_card_number string, credit_card_provider string, event_ts timestamp, latitude double, longitude double, transaction_amount long, transaction_currency string, transaction_type string").json("{0}/cicddemo/pyspark/trans/{1}/trx_batch_1".format(storageLocation, username))
...
spark.sql("SELECT * FROM spark_catalog.CICD_DB_{0}.TRANSACTIONS_{0}.history".format(username)).show()

# ICEBERG TABLE SNAPSHOTS (USEFUL FOR INCREMENTAL QUERIES AND TIME TRAVEL)
spark.sql("SELECT * FROM spark_catalog.CICD_DB_{0}.TRANSACTIONS_{0}.snapshots".format(username)).show()
